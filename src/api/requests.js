...
export const fetchOrgUnitLevels = async () => {
  const resp = await dhis2Api
    .get(
      `organisationUnitLevels.json?fields=id,displayName~rename(name),level&paging=false&order=level:asc`
    )
    .json();
...
export const fetchCurrentAttributes = async () => {
  const resp = await dhis2Api
    .get(
      `attributes.json?fields=id,name&filter=valueType:eq:GEOJSON&filter=organisationUnitAttribute:eq:true&paging=false`
    )
    .json();
...
export const publishCatchment = async (body) => {
  ...
  try {
    ...
    body.payload.description = Array.isArray(des)
      ? `groups: ${des.join(', ')} | ${body.user} | ${body.date}`
